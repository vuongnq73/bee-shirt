...
public class VoucherController1 {
    ...
    public ResponseEntity<List<Voucher1>> searchByDateRange(
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDateTime batdau,
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDateTime ketthuc) {
    ...
    public ResponseEntity<Void> updateVoucherStatus(@RequestParam Long voucherId) {
        ...
        Voucher1 voucher = voucherRepository.findById(voucherId)
                .orElseThrow(() -> {
                    log.error("Voucher with ID {} not found", voucherId);
                    return new ResourceNotFoundException("Voucher not found with id: " + voucherId);
                });

        // Lấy ngày và thời gian hiện tại theo múi giờ Asia/Ho_Chi_Minh
        LocalDateTime currentDateTime = LocalDateTime.now(ZoneId.of("Asia/Ho_Chi_Minh"));

        // Cập nhật trạng thái voucher dựa trên ngày và thời gian
        if (voucher.getStartdate() != null && voucher.getStartdate().isAfter(currentDateTime)) {
            voucher.setStatus_voucher(2); // Sắp hoạt động
        } else if (voucher.getEnddate() != null && voucher.getEnddate().isBefore(currentDateTime)) {
            voucher.setStatus_voucher(0); // Ngưng hoạt động
        } else {
            voucher.setStatus_voucher(1); // Hoạt động
        }

        // Log trạng thái và lưu thay đổi
        log.info("Updating voucher {} with status {}", voucherId, voucher.getStatus_voucher());
