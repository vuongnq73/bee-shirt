...
//danh sách caác đơn hàng của client có trạng thái là đã hủy
    ...
    //hiển thị danh sách đơn hàng của mail đó
    @GetMapping("/order-history")
    public ApiResponse<List<MyOderDTO>> getOrderHistory(@RequestParam String email) {
        List<MyOderDTO> orders = billService.getOrdersByEmail(email);
        return ApiResponse.<List<MyOderDTO>>builder().result(orders).build();
    }
